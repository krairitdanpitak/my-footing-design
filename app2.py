...
def process_footing_calculation(inputs):
    ...
    # [EDIT]: เน้นสีแดงที่ตัวแปร Pu (pu_tf) ในช่อง Substitution
    sub_text = f"<span style='color:red; font-weight:bold;'>{pu_tf}</span> / {n_pile}"
    row("Load per Pile (Ru)", "Pu / N", sub_text, f"{p_avg:.2f}", "tf",
...
def generate_report_html(title, rows, imgs, proj, eng, elem_id, inputs):
    ...
    <style>
        ...
        /* [EDIT]: Removed font-weight: bold from col-result */
